// Type definitions for rollup 0.49
...
export interface BundleOptions {
	 ...
	 * Can also be false (no indent), or true (the default Ð auto-indent)
	 */
	 ...
	 * This generally only applies if your external dependencies were transpiled (for example with Babel) Ð if you're sure you don't need it, you can save a few bytes with interop: false.
	 */
	interop?: boolean
...
export interface WriteOptions extends BundleOptions {
	/** The file to write to. If `options.sourceMap === true`, two files will be created Ð `dest` and `dest + '.map`. */
	dest: string
...
export interface Options {
	...
	acorn?: acorn.Options
	/** By default, the context of a module Ð i.e., the value of `this` at the top level Ð is `undefined`. In rare cases you might need to change this to something else, like `'window'`. */
	context?: any
